...
LaserProjectileData::LaserProjectileData()  : ProjectileData()
{
   beamStartRadius = 0.0;
   beamEndRadius = 0.0;
   interval = 0;
    ...
    {
        INIT_IMAGEASSET_ARRAY(materialList, GFXStaticTextureSRGBProfile, i);
    }
}
...
bool LaserProjectileData::preload(bool server, String &errorStr) {
   if (Parent::preload(server, errorStr) == false)
	...
	if(!server) {
      for (U32 i = 0; i < maxTextures; i++) {
             _setmaterialList(getmaterialList(i), i);
             //AssetPtr<ImageAsset> materialAsset = getmaterialListAsset(i);
           //  materialTextureHandles[i] = GFXTexHandle(materialAsset->);
             //materialList[i] = GFXTexHandle(materialListNames[i], &GFXStaticTextureProfile, avar("%s() - materialListNames[%d] (line %d)", __FUNCTION__, i, __LINE__));
      }		
	}
	return true;
}
//--------------------------------------------------------------------------
LaserProjectile::LaserProjectile() : Projectile()
{
   mLastTime = 0;
   mIndex = 0;
...
void LaserProjectile::renderObject(ObjectRenderInst *ri, SceneRenderState *state, BaseMatInstance* overrideMat) {
   if (overrideMat)
	...
	if (timeDelta > mDataBlock->interval) {
        ...
        if (mDataBlock->getmaterialList(mIndex) == StringTable->EmptyString())
        {
		   mIndex = 0;
		}
	}

    GFX->setTexture(0, mDataBlock->getmaterialListResource(mIndex));
